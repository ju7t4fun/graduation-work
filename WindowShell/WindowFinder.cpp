...
)
{
  ...
  wsprintfA (szText, "%d", screenpoint.x);
  SetDlgItemTextA (hwndDialog, IDC_STATIC_X_POS, szText);
  
  wsprintfA (szText, "%d", screenpoint.y);
  SetDlgItemTextA (hwndDialog, IDC_STATIC_Y_POS, szText);
...
long DisplayInfoOnFoundWindow (HWND hwndDialog, HWND hwndFoundWindow)
{
  ...
  have_hwnd = TRUE;
  hwndTargetWindow = hwndFoundWindow;
  ...
  GetClassNameA (hwndFoundWindow, szClassName, sizeof (szClassName) - 1);
  ...
  SetDlgItemTextA (hwndDialog, IDC_EDIT_STATUS, szText);
...
)
{
  ...
  switch (uMsg)
  {
	...
	case WM_COMMAND :
	{
	  ...
	  if (wID == IDOK && have_hwnd)
	  {
		  if(record_active){
			SetDlgItemText (hwndDlg, IDOK, L"Почати запис!");
			record_active = ~record_active;
			PostMessage(g_hwndMainWnd,WM_UNSET_HOOK,(WPARAM)hwndDlg,(LPARAM)hwndTargetWindow);
		  }else{
			SetDlgItemText (hwndDlg, IDOK, L"Зупинити запис!");
			record_active = ~record_active;
			PostMessage(g_hwndMainWnd,WM_SET_HOOK,(WPARAM)hwndDlg,(LPARAM)hwndTargetWindow);
		  }
